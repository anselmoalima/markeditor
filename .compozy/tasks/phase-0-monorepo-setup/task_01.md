...
## Subtasks
- [ ] 1.1 Create root `package.json` with private flag, packageManager pin, engines, and shared scripts placeholders.
- [ ] 1.2 Create `pnpm-workspace.yaml` declaring `packages/*` and `apps/*`.
- [ ] 1.3 Create `.gitignore`, `.editorconfig`, `.nvmrc`, `LICENSE` (MIT).
- [ ] 1.4 Run `pnpm install` and commit the lockfile.
- [ ] 1.5 Add a CI-callable smoke check that verifies workspace discovery (`pnpm -r --shell-mode exec true` or `pnpm m ls --json`).
...
## Tests
- Unit tests:
  - [ ] `pnpm m ls --json` returns valid JSON containing exactly one root and zero workspaces yet (workspaces added in later tasks).
  - [ ] Running `pnpm install --frozen-lockfile` on the committed lockfile exits 0 with no warnings.
  - [ ] `node -e "require('./package.json').packageManager.startsWith('pnpm@9')"` exits 0.
- Integration tests:
  - [ ] Fresh clone in temp dir + Corepack-enabled shell: `corepack pnpm install` succeeds without prompting for pnpm download confirmation.
