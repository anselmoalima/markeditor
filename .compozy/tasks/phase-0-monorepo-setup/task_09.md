...
- Creating a changeset against `markeditor` MUST produce a markdown file under `.changeset/`.
...
- [x] 9.4 Verify `pnpm changeset` creates a draft file and `pnpm changeset version` correctly bumps `packages/markeditor` only.
...
- Integration test: dry-run `changeset version` bumps only `markeditor` **(REQUIRED)**.
...
- Integration tests:
  - [x] Create a temp changeset declaring a `patch` bump for `markeditor`; running `pnpm changeset version` bumps `packages/markeditor/package.json` and does NOT touch `apps/playground/package.json`.
...
- All tests passing
- Test coverage >=80%
- Changesets recognizes `markeditor` and ignores `playground`.
