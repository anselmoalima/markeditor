...
## Subtasks
- [ ] 4.1 Scaffold `packages/markmd/` with `src/`, `tests/` placeholder, `package.json`, `tsconfig.json` extending base.
- [ ] 4.2 Implement `src/types.ts`, `src/index.ts`, `src/MarkmdEditor.tsx`, `src/styles/index.css` per TechSpec Core Interfaces.
- [ ] 4.3 Add `tsup.config.ts` producing dual ESM/CJS + d.ts + CSS bundle.
- [ ] 4.4 Configure `exports` map with subpaths; add a `src/plugins/index.ts` stub so the subpath resolves cleanly.
- [ ] 4.5 Verify `pnpm --filter markmd build` produces `dist/index.mjs`, `dist/index.cjs`, `dist/index.d.ts`, `dist/index.d.cts`, `dist/styles.css`, `dist/plugins/index.*`.
...
## Tests
- Unit tests:
  - [ ] `import { MarkmdEditor, MarkmdEditorProps, MarkmdEditorRef, MarkmdMode } from 'markmd'` (sourced from `src/index.ts`) resolves at TS level.
  - [ ] `MarkmdEditor` is a `forwardRef` component (has `$$typeof` ForwardRef tag).
  - [ ] Rendering `<MarkmdEditor />` with `@testing-library/react` produces an element with `data-testid="markmd-editor"`.
  - [ ] Calling `ref.current.getValue()` on a mounted editor returns the initial `defaultValue` or empty string.
- Integration tests:
  - [ ] After build, `node --input-type=module -e "import('./packages/markmd/dist/index.mjs').then(m => process.exit(m.MarkmdEditor ? 0 : 1))"` exits 0.
  - [ ] After build, `node -e "const m = require('./packages/markmd/dist/index.cjs'); process.exit(m.MarkmdEditor ? 0 : 1)"` exits 0.
  - [ ] `dist/styles.css` is non-empty and resolvable via the `./styles` subpath.
