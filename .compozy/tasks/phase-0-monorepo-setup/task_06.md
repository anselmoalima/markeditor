...
## Subtasks
- [ ] 6.1 Install `size-limit`, `@size-limit/preset-small-lib`, `publint`, `@arethetypeswrong/cli` at the root.
- [ ] 6.2 Create `packages/markmd/size-limit.json` with Phase 0 limits.
- [ ] 6.3 Add root scripts `size`, `publint`, `attw` delegating to the package.
- [ ] 6.4 Verify all three gates pass on the current `dist/` output.
...
## Tests
- Unit tests:
  - [ ] `size-limit.json` declares at least one entry pointing at `dist/index.mjs` with a `limit` field in KB.
  - [ ] `size-limit.json` declares an entry for `dist/plugins/index.mjs` with limit `<10 KB`.
  - [ ] Root `package.json` exposes scripts `size`, `publint`, `attw`.
- Integration tests:
  - [ ] `pnpm size` on built `dist/` exits 0 (placeholder is far below limits).
  - [ ] `pnpm publint` on built `dist/` exits 0 with zero errors.
  - [ ] `pnpm attw --pack packages/markmd` exits 0 with no resolution errors for ESM, CJS, or types.
  - [ ] Artificially bloating `dist/index.mjs` to >80KB makes `pnpm size` exit non-zero (run via a temp fixture, then revert).
