...
Phase 0 delivers the **NPM-ready foundation** for `markmd`: a pnpm + Turborepo monorepo with one publishable package (`packages/markmd`) and one private demo app (`apps/playground`), a dual ESM/CJS build via tsup, a Vitest + Playwright test stack with v8 coverage and axe-core, Changesets-driven releases with NPM provenance via OIDC, and a GitHub Actions CI matrix (Node 18/20/22 × React 18/19) that enforces lint, types, tests, bundle size, and package validity on every PR. No library feature code is written in Phase 0 — only an empty `<MarkmdEditor>` shell sufficient to prove the build pipeline end-to-end (build → publish-pack → consume in playground → render in Playwright).
...
The library exposes a stable public surface from `packages/markmd/src/index.ts`. In Phase 0 only the **placeholder shell** is implemented; the type contract below is what later phases must satisfy and what type-tests pin from day one.

```ts
// packages/markmd/src/types.ts (Phase 0 — minimal stable surface)
...
```ts
// packages/markmd/src/index.ts (Phase 0 entry)
export { MarkmdEditor } from './MarkmdEditor';
export type { MarkmdEditorProps, MarkmdEditorRef, MarkmdMode } from './types';
```

```ts
// packages/markmd/src/MarkmdEditor.tsx (Phase 0 placeholder — render only)
...
- `package.json` exports map for `packages/markmd`:
  ```json
  {
    "name": "markmd",
    "type": "module",
...
Not applicable — `markmd` is a client-side library; no HTTP surface.
...
- **Phase 0 scope (smoke only):**
  - `MarkmdEditor.test.tsx` — renders without crashing, exposes `data-testid="markmd-editor"`.
  - `index.test.ts` — `import { MarkmdEditor } from 'markmd'` resolves and is a function.
...
- **Type-level tests** (`tests/type/*.test-d.ts`) using `expect-type` — pin `MarkmdEditorProps`, `MarkmdEditorRef`, `MarkmdMode` shape so accidental breaking changes fail `tsc --noEmit`.
- **Package smoke (CI step):** `pnpm pack` the built library into a tarball, install it into a temp dir, `node -e "require('markmd')"` + `node --input-type=module -e "import('markmd')"`. Asserts the exports map works for both module systems. (Belt-and-braces alongside the Playwright check.)
...
- **Playwright** in `apps/playground/e2e/smoke.spec.ts`:
  ...
  - Navigate to `/`, assert `[data-testid="markmd-editor"]` is visible, no console errors.
...
6. **`packages/markmd` skeleton** — `src/index.ts`, `src/MarkmdEditor.tsx`, `src/types.ts`, `src/styles/index.css`, `package.json` with `exports` map, `tsconfig.json` extending base. Depends on steps 1–2.
7. **tsup build config** — `tsup.config.ts` producing ESM + CJS + d.ts + CSS. Depends on step 6.
8. **Vitest config + smoke tests** — `vitest.config.ts`, `tests/unit/MarkmdEditor.test.tsx`, `tests/type/props.test-d.ts`. Depends on steps 6–7.
9. **size-limit, publint, attw configs** — `size-limit.json` in `packages/markmd`, root scripts to invoke. Depends on step 7.
10. **`apps/playground` skeleton** — Vite + React app, `"markmd": "workspace:*"`, one route rendering `<MarkmdEditor />`. Depends on step 6.
...
- **NPM org/account:** `markmd` name available; 2FA on publish account.
...
- **npm provenance attestation** visible on `npmjs.com/package/markmd` after first publish.
