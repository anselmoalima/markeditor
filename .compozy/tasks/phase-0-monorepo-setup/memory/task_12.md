...
Smoke dry-run release (0.0.0-test tag) — validate end-to-end release pipeline by publishing a test prerelease of `markmd` to npm with provenance attestation, then deprecating it.
...
1. `markmd` npm package owned by `banyawat <bunyawat.38@gmail.com>` — verified with `npm view markmd --json`
...
- `git checkout -- packages/markmd/package.json` failed (untracked file) → used Edit tool to manually reset version.
