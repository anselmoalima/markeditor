...
## Subtasks
- [ ] 10.1 Author `.github/workflows/ci.yml` with the matrix and ordered steps.
- [ ] 10.2 Add cache config for pnpm-store, `.turbo`, Playwright browsers.
- [ ] 10.3 Add React-version override step using `pnpm update react@X react-dom@X -r --no-save` (or `--save-workspace-protocol` equivalent) before tests.
- [ ] 10.4 Wire Codecov action on the primary matrix cell only.
...
- Unit tests:
  ...
  - [ ] Workflow declares `strategy.matrix.node-version: [18, 20, 22]` and `react-version: [18, 19]`.
  - [ ] Workflow contains steps invoking `pnpm install --frozen-lockfile`, `pnpm lint`, `pnpm -r test`, `pnpm size`, `pnpm publint`, `pnpm attw`, and `pnpm --filter playground e2e` in correct order.
  - [ ] Workflow contains `codecov/codecov-action@v4` step gated by an `if` matching only the primary matrix cell.
- Integration tests:
