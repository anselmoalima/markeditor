...
- [x] 3.1 Install ESLint 9 + plugins + Prettier + Husky + lint-staged at root.
- [x] 3.2 Create `eslint.config.js` with shared rules and per-workspace overrides (placeholder for `packages/*` and `apps/*`).
- [x] 3.3 Create `.prettierrc` and `.prettierignore`.
- [x] 3.4 Add `lint-staged` config to root `package.json` and `.husky/pre-commit` hook.
- [x] 3.5 Add `prepare` script to bootstrap Husky.
- [x] 3.6 Add `pnpm lint`, `pnpm format`, `pnpm format:check` root scripts wired to Turbo where applicable.
...
- Unit tests:
  - [x] `eslint.config.js` exports an array containing rules for `typescript-eslint`, `react`, `react-hooks`, `jsx-a11y`, `import-x`.
  - [x] `.prettierrc` parses and contains `singleQuote: true`, `trailingComma: "all"`, `printWidth: 100`.
  - [x] Root `package.json` `lint-staged` map covers `*.{ts,tsx,js,jsx}` and `*.{json,md,css}` patterns.
  - [x] `pnpm lint` on a fixture file with a `no-unused-vars` violation exits non-zero.
  - [x] `pnpm lint` on a clean fixture file exits 0.
- Integration tests:
  - [x] Staging a `*.ts` file with unfixable lint error and running `pnpm exec lint-staged` exits non-zero.
  - [x] Staging a `*.ts` file with a fixable formatting issue and running `pnpm exec lint-staged` rewrites the file and stages the fix.
