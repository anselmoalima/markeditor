...
## Subtasks
- [ ] 2.1 Add `tsconfig.base.json` at repo root with strict compiler options.
- [ ] 2.2 Add `turbo.json` with pipelines and per-pipeline `outputs` declarations.
- [ ] 2.3 Add `turbo` to root devDependencies and wire root scripts to delegate.
- [ ] 2.4 Verify Turbo discovers zero workspaces gracefully (no-op build).
...
## Tests
- Unit tests:
  - [ ] `tsconfig.base.json` parses as valid JSON and contains `strict: true`, `noUncheckedIndexedAccess: true`, `exactOptionalPropertyTypes: true`.
  - [ ] `turbo.json` parses and declares `build`, `test`, `lint`, `typecheck`, `size`, `e2e` tasks.
  - [ ] `turbo.json` does NOT declare a `remoteCache` config (local-only).
  - [ ] Root `package.json` exposes scripts `build`, `test`, `lint`, `typecheck`, `size`.
- Integration tests:
  - [ ] `pnpm turbo run typecheck --dry=json` returns a valid plan with zero tasks (no workspaces yet).
  - [ ] `pnpm turbo run build` exits 0 on empty workspace set.
