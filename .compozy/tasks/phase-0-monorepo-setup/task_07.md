...
## Subtasks
- [ ] 7.1 Scaffold `apps/playground` with `package.json`, `vite.config.ts`, `tsconfig.json` (extends base), `index.html`.
- [ ] 7.2 Implement `src/main.tsx` mounting `<App />` and `src/App.tsx` rendering `<MarkmdEditor />`.
- [ ] 7.3 Import `markmd/styles` from `App.tsx`.
- [ ] 7.4 Verify dev/build/preview commands all work locally.
- [ ] 7.5 Wire playground tasks into Turbo (`build`, `preview`) with `dependsOn: ["markmd#build"]`.
...
## Tests
- Unit tests:
  - [ ] Rendering `<App />` via `@testing-library/react` shows an element with `data-testid="markmd-editor"`.
  - [ ] `App.tsx` imports `markmd/styles` (asserted via spying on Vite import graph or static grep test).
  - [ ] `apps/playground/package.json` declares `"markmd": "workspace:*"` and is marked `private: true`.
- Integration tests:
  - [ ] `pnpm --filter playground build` exits 0 and produces `dist/index.html`.
  - [ ] `pnpm --filter playground preview` serves the built bundle on a free port and responds 200 to `GET /`.
  - [ ] Symlink validation: `node -e "require.resolve('markmd', { paths: ['apps/playground'] })"` resolves to the workspace package, not a hoisted copy.
