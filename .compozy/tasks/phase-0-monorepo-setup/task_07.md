...
Scaffold `apps/playground` as a private Vite + React app that imports `markeditor` via `"markeditor": "workspace:*"`. This proves the workspace symlink works end-to-end and gives Playwright a target to drive in task_08. The app is never published — it is a demo / smoke harness.
...
<requirements>
- `apps/playground/package.json` MUST set `"private": true`, `"type": "module"`, and `"markeditor": "workspace:*"` in dependencies.
- App MUST be Vite + React 18 (or 19, configurable later in CI matrix); single `index.html`, `src/main.tsx`, `src/App.tsx`.
- `src/App.tsx` MUST render `<MarkEditor />` imported from `'markeditor'`, with `import 'markeditor/styles'`.
...
- [x] 7.2 Implement `src/main.tsx` mounting `<App />` and `src/App.tsx` rendering `<MarkEditor />`.
- [x] 7.3 Import `markeditor/styles` from `App.tsx`.
- [x] 7.4 Verify dev/build/preview commands all work locally.
- [x] 7.5 Wire playground tasks into Turbo (`build`, `preview`) with `dependsOn: ["markeditor#build"]`.
...
- `apps/playground/package.json` — workspace dep on `markeditor`.
...
- `packages/markeditor` (task_04) is consumed via symlink.
...
- Updated `turbo.json` `dependsOn` for `playground#build` → `markeditor#build`.
...
- Unit tests:
  - [x] Rendering `<App />` via `@testing-library/react` shows an element with `data-testid="mark-editor"`.
  - [x] `App.tsx` imports `markeditor/styles` (asserted via spying on Vite import graph or static grep test).
  - [x] `apps/playground/package.json` declares `"markeditor": "workspace:*"` and is marked `private: true`.
- Integration tests:
  ...
  - [x] Symlink validation: `node -e "require.resolve('markeditor', { paths: ['apps/playground'] })"` resolves to the workspace package, not a hoisted copy.
...
- Playground bundle correctly resolves the workspace `markeditor` package.
